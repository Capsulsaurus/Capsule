...
| `capsule-sdk`             | Client SDK: the sanctioned network path (auth/session + token refresh, upload protocol, sync, LAN-peering glue); typed REST client generated by `spargen` from OpenAPI 3.1 |
...
| `capsule-web`             | Browser client: guest drop sealing + upload, the share-link viewer, and a key-free authenticated read path over the gRPC-web sync feed |
| `capsule-wasm`            | The browser WASM surface `capsule-web` loads (share-link open, drop sealing) — built by `mise run build-wasm`, never committed |
...
| `crypto::authority::openmls_authority`                                  | [Cryptography — MLS](/design/cryptography/mls/), [MLS Resilience](/design/mls-resilience/)                     | Unit + Smoke (protocol round-trip)            |
...
| `import::{scanner,planner,executor,plan,upload,streaming,group,progress,special,importers}` | [Import — Pipeline](/design/import/pipeline/)                                             | Unit (planner determinism) + Smoke (executor) |
...
| `ml`                                                                    | [AI/ML Integrations](/design/ai/)                                                                             | Unit + Smoke (inference parity per-platform)  |
| `sharing`                                                               | [Share Links](/design/share-links/)                                                                            | Unit                                          |
| `drop`                                                                  | [Web Upload](/design/web-upload/)                                                                              | Unit (seal round-trip + adoption rewrap)      |
...
| `peering` (the mDNS responder is a mocked seam; a live responder is post-v1) | [Peering](/design/peering/)                                              | Unit + Smoke per platform             |
...
| `capsule-api-auth::devices`                          | [Device Enrollment](/design/device-enrollment/)                                      | Smoke                                       |
| `capsule-api-upload`                                 | [Import — Upload Protocol](/design/import/upload-protocol/)                          | Unit + Smoke + 1 E2E                        |
| `capsule-api-upload::ops`                            | [Authorization](/design/authorization/#the-lifecycle-write-surface), [Threat Model — Validation](/design/threat-model/validation/) | Unit + Smoke (+ E2E case 7)                 |
| `capsule-api-media::routes`                          | [Filesystem — Server](/design/filesystem/server/), [Thumbnails](/design/thumbnails/) | Smoke                                       |
| `capsule-api-media::verify`                          | [Import — Storage Verification](/design/import/storage-verification/) | Unit + Smoke                                |
| `capsule-api-media::shares`                          | [Share Links](/design/share-links/)                                                  | Unit + Smoke                                |
| `capsule-api-media::drops`                           | [Web Upload](/design/web-upload/)                                                    | Unit + Smoke                                |
...
| `capsule-api-service::quota`                         | [Quota](/design/quota/)                                                              | Unit                                        |
| `capsule-api-service::moderation` (the `/blob/{hash}` takedown 410 gate is wave-2 `S-C17`) | [Moderation](/design/moderation/)               | Unit + Smoke                                |
...
| `capsule-core::media` (behind the non-default `media` feature) | [Thumbnails and Previews](/design/thumbnails/) (decode/encode utilities + signed derivative generation; **JPEG is the only wired codec** — PNG's decode/encode return a typed error, and the remaining format shims are stubs that wave-2 slice `S-B13` converts into typed `UnsupportedFormat` rejections) | Unit                 |
...
| [Cryptography — Keys: Write Authority Interface](/design/cryptography/keys/#write-authority-interface) | `capsule-core::crypto::authority` (`AlbumAuthority` trait, the offline `ReferenceAuthority` epoch ledger, and the live `OpenMlsAuthority`) |
| [Cryptography — MLS](/design/cryptography/mls/)                     | `capsule-core::crypto::authority::openmls_authority` (the doc still names the module `crypto::mls`; the wrapper shipped under `crypto::authority`) |
...
| [MLS Resilience](/design/mls-resilience/)                           | `capsule-core::crypto::authority::openmls_authority::resilience` (+ `::upgrade` for the ceremony)                              |
...
| [Authorization](/design/authorization/)                             | `capsule-api-auth::roles`, `capsule-core::crypto::provenance` (verify_asset), `capsule-api-upload::ops` (lifecycle-write surface) |
...
| [Versioning](/design/versioning/)                                   | Cross-cutting: `capsule-api` (header enforcement), `capsule-core::crypto::authority::openmls_authority::upgrade` (upgrade ceremony), `capsule-api-migration` |
| [Backup and Recovery](/design/backup-recovery/)                     | `capsule-core::backup`, `capsule-api-auth::routes::escrow` (master-key escrow surface)                                       |
...
| [Import — Pipeline](/design/import/pipeline/)                       | `capsule-core::import::*` (including the streaming import-upload loop and the `library::available_bytes` free-space probe); the only third-party source adapter is `import::importers::takeout`, whose sidecar-enrichment fold and CLI entry point are wave-2 (`S-B10`/`S-B11`) |
| [Import — Camera Import](/design/import/camera-import/)             | `capsule-core::import::camera` (planned, post-v1) over the gated in-house `ptpip-rs` crate                                    |
...
| [Peering](/design/peering/)                                         | `capsule-sdk::peering` (mDNS responder still a mocked seam) + `capsule-core::backup` (artifact format)                        |
| [Organization](/design/organization/)                               | `capsule-core::domain::stack_type`, `capsule-api-service::{album,stack}`                                                      |
| [AI/ML Integrations](/design/ai/)                                   | `capsule-core::ml` (registry, vector index, orchestration; the CLIP runner is behind the default-off `inference` feature), `capsule-vision` (Python research) |
| [Thumbnails](/design/thumbnails/)                                   | Client-side generation in `capsule-core::media::image::derivative` (behind the `media` feature) + serving in `capsule-api-media`     |
| [Share Links](/design/share-links/)                                 | `capsule-core::sharing`, `capsule-api-media::shares`, `capsule-wasm` + `capsule-web` (the viewer)                             |
| [Web Upload](/design/web-upload/)                                   | `capsule-core::drop`, `capsule-api-media::drops`, `capsule-wasm` + `capsule-web`; reuses `capsule-api-upload` for drop chunks  |
| [Moderation](/design/moderation/)                                   | `capsule-api-service::moderation` (`report`, `blocklist`, `takedown`, `suspension`)                                            |
| [Quota](/design/quota/)                                             | `capsule-api-service::quota` + `capsule-api-upload::routes::quota`                                                             |
...
**Status (2026-08-21):** **seven of the 13 cases carry a live marker** in the test suite — cases **1, 4, 5, 8, 9, 10, and 13** (`rg "E2E case"` locates each one). Cases 8, 10, and 13 run in an in-process or KAT shape rather than against a live browser or a second physical device; that is the shape the repo can run today, and each marker says so at its site. The remaining six are wave-2 work: cases **2, 3, and 11** are slice `S-Q1`, case **6** is `S-Q2`, case **7** is `S-Q3`, and case **12** is `S-Q4`. The live-browser halves of cases 5 and 13 are `S-Q5`. The bound now governs: adding a fourteenth case requires amending this list and the owning doc's Validation section.

1. **Auth → sync → client-side library query.** Log in via OIDC → access token → gRPC `Sync` returns the account's album entries → the client applies them and a local `library.sqlite` query lists the expected albums (rich queries are client-side per [API Surfaces](/design/api-surfaces/)). Covers `capsule-api-auth::oidc + session` × `capsule-api-sync` × `capsule-core::library`. Made live by slice `S-D5`.
