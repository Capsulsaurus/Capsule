...
- **Single verification chokepoint.** All of this lives in one `verify_asset(manifest, ciphertext, authority)` function in `capsule-core::crypto`, where `authority` is the album's [`AlbumAuthority`](#write-authority-interface). It is the only path by which a client acknowledges an asset, and per [contract-driven development](/design/principles/) it ships with exhaustive negative test cases: reader-signed, removed-writer, wrong-epoch, forged certificate chain, replayed manifest.
