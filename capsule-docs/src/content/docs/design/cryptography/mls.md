...
The `OpenMlsAuthority` wrapper has landed (slices `S-X1`–`S-X3`), so the epoch-authority role this doc assigns to the MLS commit chain is filled live: `OpenMlsAuthority` implements the [`AlbumAuthority` interface](/design/cryptography/keys/#write-authority-interface) without touching `verify_asset`, and the membership ceremonies, `Welcome`/history delivery, and the [album upgrade ceremony](/design/versioning/#album-upgrade-ceremony) are implemented and tested. The offline admin-signed epoch ledger (`ReferenceAuthority`) is retained behind the same interface as the deterministic fixture path. The per-user-block `Remove` + epoch bump landed with it (`S-X4`): `OpenMlsAuthority::block_user` removes every one of the blocked user’s devices in a single `Remove` + `Commit`, so the AMK epoch bumps once and the write-tier key rotates once. Docs that depend on live MLS link to this note rather than restating it.
