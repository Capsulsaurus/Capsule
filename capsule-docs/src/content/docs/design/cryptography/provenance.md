...
**Physical placement.** The manifest is **not** embedded inside the ciphertext blob. Every ciphertext blob — original, derivative, metadata — is an independent, fully opaque content-addressed object, and the signed manifest travels beside them as **one more blob in the same bundle**: the asset's **envelope object**, uploaded under the `provenance` [blob role](/design/import/upload-protocol/#what-gets-uploaded) through the same session machinery as every other blob. It is a small, deliberately server-visible signed CBOR object (it contains no plaintext secrets by construction) and the one member of a bundle that is not ciphertext. The server stores those bytes verbatim at their content address — the copy that makes the index [reconstructible without PostgreSQL](/design/filesystem/server/#recovering-the-index-from-blobs-alone) — and separately projects the fields it must read into the PostgreSQL rows the hot path queries. The client persists the same manifests inside its local provenance chain file. Per blob role, the envelope's associations are by hash: the **original** and each **derivative** are named by `ciphertext_hash` in their (Asset/Derivative) manifest, and the **metadata blob** by the manifest's `metadata_blob_hash`. No ciphertext blob carries an embedded header.

**The signed bytes are the served bytes.** The manifest is uploaded, never synthesized: no server-side surface — the [sync feed](/design/import/download-sync/#discovering-what-changed), an [index rebuild](/design/filesystem/server/#recovering-the-index-from-blobs-alone), a [federation pull](/design/federation/) — may re-encode a manifest from the PostgreSQL projection and present the result as the asset's manifest. The projection holds no signature, so a re-serialization is unverifiable by construction: a recipient handed one cannot tell an authentic write from a fabricated one, which is the whole property the manifest exists to provide. It is also unverifiable *in principle* rather than merely in practice — the signatures cover a specific canonical encoding down to the wire-presence rules below, so any encoder other than the signer's is guessing at a byte-exact target. Carrying the manifest as a blob gets the rest for free: identical manifests share one content address and dedupe, the object is immutable like every other blob, and nothing new appears on the wire.
...
- **Append-only enforcement (cryptographic, not just storage).** The guarantee is the signature chain, not the file mode. A unit test drops or rewrites a record in a serialized chain and asserts the forward walk from `create` detects the break (a non-matching prior hash, or a signature that no longer verifies). A companion test confirms the server rejects any overwrite or delete of an existing provenance entry, and that a client whose local `.provenance.cbor` has been tampered re-derives the authoritative chain from the server rather than trusting the local bytes. Append-only is *derived* rather than asserted: [invariant 17](/design/threat-model/validation/#in-the-index-critical-section-every-manifest-write-upload-or-lifecycle) refuses a manifest that does not advance the chain, and [invariant 16](/design/threat-model/validation/#in-the-index-critical-section-every-manifest-write-upload-or-lifecycle) closes the action enum — which contains no overwrite or delete action for a provenance entry, so there is no surface through which one could be requested.
...
- **Manifest bytes survive the round trip (smoke).** A manifest uploaded as a `provenance` blob is served back byte-identical by every surface that serves it — sync feed, blob fetch, index rebuild — and `verify_asset` accepts the received bytes without re-encoding them. The companion negative feeds a manifest re-serialized server-side from the envelope projection and asserts it is rejected at the signature, so a regression to re-encoding fails loudly instead of quietly serving unverifiable bytes.
