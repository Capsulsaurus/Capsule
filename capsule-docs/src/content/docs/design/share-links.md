...
**Status note.** The serve path, the owner's issue and revoke operations, and the indistinguishable `404` are **served today** by the Kynos surface, with slice `S-C4`. Three qualifications, all recorded rather than implied:

- **The mandatory strip is the issuing client's, not the serve path's** — see `S-C50`. A share's metadata is ciphertext sealed under material derived from the URL fragment, which never reaches the server, so there is nothing on the serve path to redact; a server that *could* strip a share could read every share. What the server enforces is the containment property that makes the client's strip stick: **a link serves only the addresses its own record enumerates**, so a share cannot be walked sideways into the album's unstripped metadata. The no-opt-out rule survives unchanged and binds the issuing client.
