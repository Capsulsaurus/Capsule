...
**What a revoke is immediate about.** Every session record is closed, so no refresh token can mint anything from that moment. An access token already in flight is a signed bearer credential with its own deadline and outlives the revoke by at most its remaining lifetime — which is why that lifetime is fifteen minutes, and why slice `S-C48` is where making the check ledger-backed is decided rather than assumed.
