...
| MLS group layer | `openmls` (`libcrux-provider`) + `openmls_libcrux_crypto` + `openmls_basic_credential` + `openmls_traits` | `capsule-core::crypto::authority::OpenMlsAuthority` — the live RFC 9420 MLS backend (slice S-X1), pinned to the X-Wing PQ ciphersuite `MLS_256_XWING_CHACHA20POLY1305_SHA256_Ed25519` (`0x004D`). The libcrux provider is the only released one shipping the X-Wing suite (formally-verified ML-KEM + X25519 from Cryspen). Host-only, behind the `mls` feature (implied by `native`); the wasm32 sealing build excludes it — libcrux does not target `wasm32-unknown-unknown`. The offline `ReferenceAuthority` stays available on every build. | Not `mls-rs`: no third-party audit and no PQ suite (see [Cryptography — MLS](/design/cryptography/mls/)). `0x004D` is a private/experimental codepoint (no IANA number) — acceptable for Capsule's closed deployment; a future move to a WG-standardized ML-KEM-hybrid suite rides the album upgrade ceremony (S-X3). |
