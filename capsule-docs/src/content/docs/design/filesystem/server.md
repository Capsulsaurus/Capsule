...
Implementation is planned in `capsule-api::{blob,index}` behind a Capsule-owned blob-store port with arbitrary backend support. The session-state store is a [deployment choice](#deployment-profiles), not a versioned API surface.
