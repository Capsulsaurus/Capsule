...
- **`deep`** (default `false`) asks the server to re-read and re-hash the blob bytes rather than trusting the `stat` + index, catching silent bit-rot at the cost of I/O. It is opt-in because the structural check answers the common case (did finalization actually durably land?) cheaply, while a deep scan is for periodic integrity audits. `deep` is a server-priced operation: rate-limited per user and coalesced server-side (a repeated `deep` request against the same blob within a window returns the cached result), so a client cannot turn it into an I/O-amplification attack.
  ...
  **Status note (`S-C41`).** `deep` is **served today** by the Kynos surface, streamed a mebibyte at a time so a deep verify of a large original is not a large allocation, and rate-limited per account — the limiter is half the feature rather than a refinement of it, since an unbounded deep scan is an I/O-amplification attack costing the caller one small JSON body. Two shapes are worth knowing: a throttled deep scan returns the **structural** verdict with each blob's `deep` reading `rate_limited`, rather than refusing the request, because refusing would make the limiter cost a client more than it saves; and `deep` is **absent** when no scan ran, which is deliberately distinguishable from a scan that found the bytes fine. **Coalescing is deliberately not implemented** — a coalesced verdict is a durability claim with an age, and handing a client a minute-old integrity result as a current one is exactly the mistake this surface exists to prevent. If it is ever added, the window has to appear in the verdict.
...
- **Fetched** via `GET /v1/upload/{id}/receipt` in the session window (pairing with the [lost-ACK recovery flow](/design/import/upload-protocol/#idempotency-and-resumption)) and `GET /v1/assets/{asset_id}/receipts` durably. Receipts are permanent objects, exempt from session GC. The durable fetch is **owner-only** (slice `S-C58`, narrowing an earlier "uploader or owner"): the case that made the uploader admissible is a guest depositing through a [drop link](/design/web-upload/), and a guest who has given the asset up must not learn when the owner later amended, culled or restored it. A caller who is not the owner gets the same `404 error.storage.asset_not_found` an unknown asset gets, so a guess reveals nothing.
...
- **Durable verdict (smoke).** Upload an asset to completion; `POST /v1/storage/verify` with its blob hashes; assert `durable = true` and every blob `stored && indexed && retrievable`.
