...
- **Fetched** via `GET /upload/{id}/receipt` in the session window (pairing with the [lost-ACK recovery flow](/design/import/upload-protocol/#idempotency-and-resumption)) and `GET /assets/{asset_id}/receipts` durably. Receipts are permanent objects, exempt from session GC. The durable fetch is **owner-only** (slice `S-C58`, narrowing an earlier "uploader or owner"): the case that made the uploader admissible is a guest depositing through a [drop link](/design/web-upload/), and a guest who has given the asset up must not learn when the owner later amended, culled or restored it. A caller who is not the owner gets the same `404 error.storage.asset_not_found` an unknown asset gets, so a guess reveals nothing.
