...
The upload protocol is Capsule-owned because session state, encrypted-blob provenance, finalization, and future access semantics are one end-to-end security contract. It uses the useful offset/PATCH shape of [TUS](https://tus.io/) v1, but TUS is an implementation reference rather than a compatibility target or a reason to extract a generic transfer library: Capsule permits no per-request capability negotiation, metadata smuggled in headers, or plaintext payloads. Compatibility is gated once, up front, via the universal [protocol handshake](/design/threat-model/validation/#protocol-and-capability-negotiation).

This protocol is the most fragile contract between client and server: a client that misunderstands chunk alignment, offset semantics, or finalization can silently corrupt or orphan data. The endpoint table, chunk rules, session state machine, and finalization steps below **are the contract** — every implementation MUST conform exactly. Implementations are planned in `capsule-sdk::upload` and `capsule-server::upload`; they will be tested independently against shared protocol fixtures.
...
- The **provenance blob** — the asset's [signed manifest](/design/cryptography/provenance/#asset-manifest), exactly as the client signed it. It is the one member of a bundle that is not ciphertext: deliberately server-visible signed CBOR, carrying no plaintext secrets by construction, stored and later served byte-for-byte.

Each blob is its own upload with its own upload ID; the protocol does not couple them and imposes **no wire ordering**. Every session declares its blob's **role** at creation — a closed enum `original | derivative | metadata | provenance | backup` in the `POST /v1/upload` body, recorded on the pending row — so the server can reason about bundle completeness without reading plaintext. The `provenance` role is the manifest's: an asset bundle always contains one, because the manifest is transferred like everything else rather than assembled server-side. The client may transfer the bundle in any order — decoupling lets small derivatives land while a large original is still in flight — and the server **gates visibility** on the pending-asset row: the asset becomes visible to other devices once its **manifest and metadata blob are finalized**. Whether the original is also held yet travels as a per-asset completeness fact on the sync feed (`original_held`), which is what makes [staged uploads](/design/import/download-sync/#upload-tiering-staged-uploads) possible; an asset whose original has not landed is in the derived `awaiting-original` state, and fetching its original returns the transient `error.blob.pending_upload` rather than `410`. Every blob in the bundle — original, derivatives, metadata, provenance — counts toward the uploader's storage [quota](/design/quota/#accounting-model).

**`create` and `replace` are the two actions that ride this surface, and they are not assembled the same way (slice `S-C43`).** A write that moves blob bytes is an upload by definition, so `create` and `replace` open sessions here and every other action is a [lifecycle write](/design/authorization/#the-lifecycle-write-surface). The paragraph above describes a `create`, whose bundle is assembled incrementally in a **pending** row nobody can see; the ordering freedom is safe precisely because there is no observable half-applied state. A `replace` mutates an asset that is **already visible**, and a window in which the new original is referenced by the old manifest is a window in which [`verify_asset`](/design/cryptography/keys/#write-authorization) fails for every client that fetches it. So:

- **A replace is applied as one act, when its manifest lands.** The other members commit their bytes into the content-addressed store and change nothing the index or the feed can see. This makes the manifest the member that lands **last** — the one ordering rule this protocol has, scoped to `replace`.
- **The manifest names the bundle it commits to**, through `original_blob_hash` on the `POST /v1/upload` projection. That field exists because `manifest_envelope.ciphertext_hash` names *the blob this session is uploading* rather than the manifest's own — a conflation that is invisible for a create (no member has to name another) and blocking here. It is optional on the wire and required on a replace.
- **A manifest that arrives early is refused `409 error.upload.replace_incomplete` and retried**, not rejected as malformed: the request is not wrong, it is early. A replace whose named blobs are not all present would otherwise be applied as a dangling reference — the shape the [integrity scrub](/design/filesystem/server/) exists to report — created on purpose.
- **Invariants 17 and 18 are decided by the index**, in the same critical section that re-points the roles and mints the sequence number, exactly as they are for a lifecycle write. A gate-side check would let two concurrent replaces both pass and double-apply, which is the stale revival invariant 17 exists to catch, reintroduced by the code enforcing it. A stale chain is `409 error.upload.stale_revival`.
- **An abandoned replace costs nothing permanent.** Its committed-but-unreferenced blobs are reclaimed by the collector on its ordinary schedule and the quota is credited back.

"Blob" is defined once, in [Filesystem — Server: Uniform, Opaque Blobs](/design/filesystem/server/#uniform-opaque-blobs); this protocol is its transport, not its definition. Every asset and derivative blob is covered by a signed [manifest](/design/cryptography/provenance/#asset-manifest), and that manifest reaches the server along two paths that must not be confused. The `manifest_envelope` field on the `POST /v1/upload` JSON body is a **projection** — the manifest fields the server has to read to validate a write with no key, so that at session creation it can check `created_by_device` against the uploader's [device directory](/design/cryptography/keys/#device-directory) (invariant 7) and re-check the lot at finalization. The signed CBOR *itself* is the provenance blob, transferred as bytes and stored as bytes. Only the blob carries the signatures the client verifies on download via [`verify_asset`](/design/cryptography/keys/#write-authorization); the projection carries none, so the server may never serve the projection, or anything re-encoded from it, where a manifest is expected — the verbatim rule is owned by [Provenance — The signed bytes are the served bytes](/design/cryptography/provenance/#asset-manifest). **Backup artifacts are the one exception** — they carry no per-asset provenance of their own (the exporting device is not the original author); their integrity rides the library-level backup MANIFEST instead (see [Backup and Recovery](/design/backup-recovery/)).

Carrying the manifest as a blob costs **no new wire surface**: the bundle already transfers an arbitrary number of blobs per asset, in any order, so the manifest needs no endpoint, header, error code, or state of its own, and it inherits content-addressing (identical manifests dedupe to one object) and immutability from the blobs beside it. It also makes the visibility gate above literal — "manifest and metadata blob are finalized" is two finalized sessions, not one session plus a JSON field. What finalization does with the projection once the blob is present — re-derive it from the stored bytes, or keep validating the declared JSON against them (`error.upload.envelope_mismatch`) — is an implementation choice this protocol does not fix; what it does fix is that the server never *produces* manifest bytes.
...
- **Content-addressed.** Every blob is identified by its [ciphertext content hash](/design/cryptography/primitives/). The plaintext hash is never transmitted to the server. (The provenance blob is addressed the same way, over the signed manifest bytes it carries: the digest is taken over stored bytes, not over what they mean, so identical manifests collapse to one object exactly as identical ciphertext does.)
...
| `GET`    | `/upload/sessions` | List the **uploader's** active sessions (keyed by `upload_user_id` — the resuming party — not `owner_id`), so a client can resume across app restarts. Optional `?status=` narrows it to one state, using the same tokens `X-Capsule-Upload-Status` carries; an unknown value is `400 error.upload.invalid_status_filter` and **never silently ignored**, because a filter that is dropped on the floor leaves the caller acting on a list they believe is filtered (slice `S-C57`).                                                                                                                                                                                                                                                          |
| `GET`    | `/upload/{id}/receipt` | Fetch the session's [`CustodyReceipt`](/design/import/storage-verification/#custody-receipts) once `Completed`. `409 error.upload.receipt_not_available` before then; receipts are permanent, so this also works after the session record itself expires — durably via `GET /v1/assets/{asset_id}/receipts`.                                                                                                    |

Creating a session writes a *pending* asset row to Postgres (`uploaded = false`) and a session record to **Valkey** (see [Filesystem — Server: Required Services](/design/filesystem/server/#required-services)). The pending row reserves the asset ID that derivative and metadata blobs reference. The session record carries everything finalization needs — declared size and hash, `content_type`, `crypto_suite_id`, `protocol_version`, `blob_role`, the manifest envelope, `album_id`/`owner_id`/`intent_id`, and the uploader — so a session is finalizable from its own record with no further client input.
...
| `X-Capsule-Suggested-Chunk-Size` | response  | `POST /v1/upload`           | yes      |
...
| ------------------------------------------- | ------------ | ---------------------------------------------- |
| Unknown JSON field on `POST /v1/upload`        | **rejected** | `400 error.upload.malformed_request`           |
...
- **After discard: uniform `404`.** A discarded, expired, or never-existent session is indistinguishable — `HEAD`/`PATCH`/`DELETE` all return `404 error.upload.session_not_found`. No tombstones, no `410`: this matches the platform-wide indistinguishable-404 rule owned by [API Surfaces — Rejection Mapping](/design/api-surfaces/#rejection-mapping). (Invariant 26 is the *drop-path instance* of that rule, not its source — [Federation](/design/federation/#federation-capabilities) scopes invariants 26–32 to the drop path, so an upload session is not one of them.) The client recovery is identical either way — re-create the session and resume from zero for that blob, retrying with backoff and halting after a bounded number of attempts.
...
`Completed` is a one-time transfer acknowledgement, not a standing durability guarantee a client can re-query later — but it is now accompanied by a durable, server-signed [`CustodyReceipt`](/design/import/storage-verification/#custody-receipts): the transferable evidence that this server accepted custody of exactly these bytes, fetched via `GET /v1/upload/{id}/receipt` and persisted client-side. After finalization, a client confirms an asset *remains* durably stored, indexed, and retrievable — the precondition for releasing its local copy — through the separate [storage-verification endpoint](/design/import/storage-verification/), which re-checks state that server-side GC, migration, or corruption could change after `Completed`.

## Idempotency and Resumption

- **Duplicate session creation** is keyed by `(owner_id, hash, album_id)` and split by state: if an **active** session for the tuple exists, `POST /v1/upload` returns it (`200`, `Location` + current `X-Capsule-Offset`) — no second session is created; if the hash is already **finalized**, the request is rejected with `409 error.upload.duplicate_blob` carrying the existing asset reference, which is the client's trigger to resolve the situation as a [merge](#deduplication-and-merge) instead of a transfer. The dedup check and the pending-row insert run inside a single PostgreSQL transaction (`SELECT ... FOR UPDATE` + `INSERT ... ON CONFLICT`), closing the TOCTOU race at the database layer.
...
| Receipt requested before `Completed`                  | `409` | `error.upload.receipt_not_available`       | 33        |
...
- **Duplicate `POST /v1/upload`** → active session returned / finalized hash `409 duplicate_blob` (see [Idempotency and Resumption](#idempotency-and-resumption)).
...
- **Provenance blob transport (smoke).** Upload a full bundle including the `provenance` blob; assert the stored object and every later read of it are byte-identical to the transmitted bytes, and that the role is recorded on the pending row so bundle completeness is answerable without reading plaintext.
