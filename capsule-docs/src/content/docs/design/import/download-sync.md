...
The download client is planned in `capsule-sdk` (per-platform glue handles cache placement and connection-class detection); the server-side sync feed and blob fetch are planned in `capsule-api::sync`. The `/sync` feed format is the **contract** other modules consume; its versioning and per-album monotonic ordering are what defeats the stale-rewind attack class.
