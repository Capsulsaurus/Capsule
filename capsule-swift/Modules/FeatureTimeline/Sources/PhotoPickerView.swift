    ...
    ///
    /// iOS only — `PHPickerViewController` is a `UIViewController` and has no
    /// macOS counterpart. ``SwiftUI/View/photoImportPicker(isPresented:onPicked:)``
    /// is the cross-platform entry point; the Mac reaches the open panel instead.
