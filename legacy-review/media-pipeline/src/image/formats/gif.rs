...
/// GIF codec — **not implemented in this build** (slice `S-B13`).
///
...
/// [`ImageFormat::Gif`] to [`SUPPORTED_IMAGE_FORMATS`](crate::media::image::types::SUPPORTED_IMAGE_FORMATS);
...
impl GifImage {
    ...
    pub const FORMAT: ImageFormat = ImageFormat::Gif;
}

impl ImageMetadataExtractor for GifImage {
...
impl Image for GifImage {
    ...
    /// Always [`ImageError::UnsupportedFormat`]: this build has no GIF encoder, so a
    /// buffer + metadata pair cannot be materialized into a GIF image. This is the funnel
...
impl ImageDecode for GifImage {
    /// Always [`ImageError::UnsupportedFormat`]: this build has no GIF decoder.
