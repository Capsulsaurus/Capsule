...
/// WebP codec — **not implemented in this build** (slice `S-B13`).
///
...
/// [`ImageFormat::WebP`] to [`SUPPORTED_IMAGE_FORMATS`](crate::media::image::types::SUPPORTED_IMAGE_FORMATS);
...
impl WebpImage {
    ...
    pub const FORMAT: ImageFormat = ImageFormat::WebP;
}
...
impl Image for WebpImage {
    ...
    /// Always [`ImageError::UnsupportedFormat`]: this build has no WebP encoder, so a
    /// buffer + metadata pair cannot be materialized into a WebP image. This is the funnel
...
impl ImageDecode for WebpImage {
    /// Always [`ImageError::UnsupportedFormat`]: this build has no WebP decoder.
