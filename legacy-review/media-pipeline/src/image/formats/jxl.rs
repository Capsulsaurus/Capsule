...
/// JPEG XL codec — **not implemented in this build** (slice `S-B13`).
///
...
/// [`ImageFormat::Jxl`] to [`SUPPORTED_IMAGE_FORMATS`](crate::media::image::types::SUPPORTED_IMAGE_FORMATS);
...
impl JxlImage {
    ...
    pub const FORMAT: ImageFormat = ImageFormat::Jxl;
}

impl ImageMetadataExtractor for JxlImage {
...
impl Image for JxlImage {
    ...
    /// Always [`ImageError::UnsupportedFormat`]: this build has no JPEG XL encoder, so a
    /// buffer + metadata pair cannot be materialized into a JPEG XL image. This is the funnel
...
impl ImageDecode for JxlImage {
    /// Always [`ImageError::UnsupportedFormat`]: this build has no JPEG XL decoder.
