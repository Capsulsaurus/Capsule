...
/// BMP codec — **not implemented in this build** (slice `S-B13`).
///
...
/// [`ImageFormat::Bmp`] to [`SUPPORTED_IMAGE_FORMATS`](crate::media::image::types::SUPPORTED_IMAGE_FORMATS);
...
impl BmpImage {
    ...
    pub const FORMAT: ImageFormat = ImageFormat::Bmp;
}
...
impl Image for BmpImage {
    ...
    /// Always [`ImageError::UnsupportedFormat`]: this build has no BMP encoder, so a
    /// buffer + metadata pair cannot be materialized into a BMP image. This is the funnel
...
impl ImageDecode for BmpImage {
    /// Always [`ImageError::UnsupportedFormat`]: this build has no BMP decoder.
