...
/// AVIF codec — **not implemented in this build** (slice `S-B13`).
///
...
/// [`ImageFormat::Avif`] to [`SUPPORTED_IMAGE_FORMATS`](crate::media::image::types::SUPPORTED_IMAGE_FORMATS);
...
impl AvifImage {
    ...
    pub const FORMAT: ImageFormat = ImageFormat::Avif;
}
...
impl Image for AvifImage {
    ...
    /// Always [`ImageError::UnsupportedFormat`]: this build has no AVIF encoder, so a
    /// buffer + metadata pair cannot be materialized into a AVIF image. This is the funnel
...
impl ImageDecode for AvifImage {
    /// Always [`ImageError::UnsupportedFormat`]: this build has no AVIF decoder.
