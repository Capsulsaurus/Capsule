...
//! keys ([`idempotency`]). They mirror the client-side checks in
//! [`verify_asset`](crate::crypto::verify_asset), and the server write paths consume them
//! today — the envelope gate, the ops surface, the feed, federation pull, and the drop
//! routes all validate through here. This module used to describe those consumers as
