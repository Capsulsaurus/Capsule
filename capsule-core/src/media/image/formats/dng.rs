...
/// Adobe DNG codec — **not implemented in this build** (slice `S-B13`).
///
...
/// [`ImageFormat::Raw(RawImageFormat::Dng)`] to [`SUPPORTED_IMAGE_FORMATS`](crate::media::image::types::SUPPORTED_IMAGE_FORMATS);
...
impl DngImage {
    ...
    pub const FORMAT: ImageFormat = ImageFormat::Raw(RawImageFormat::Dng);
}
...
impl Image for DngImage {
    ...
    /// Always [`ImageError::UnsupportedFormat`]: this build has no Adobe DNG encoder, so a
    /// buffer + metadata pair cannot be materialized into a Adobe DNG image. This is the funnel
...
impl ImageDecode for DngImage {
    /// Always [`ImageError::UnsupportedFormat`]: this build has no Adobe DNG decoder.
