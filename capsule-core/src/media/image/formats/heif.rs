...
/// HEIF/HEIC codec — **not implemented in this build** (slice `S-B13`).
///
...
/// [`ImageFormat::Heic`] to [`SUPPORTED_IMAGE_FORMATS`](crate::media::image::types::SUPPORTED_IMAGE_FORMATS);
...
impl HeifImage {
    ...
    pub const FORMAT: ImageFormat = ImageFormat::Heic;
}
...
impl Image for HeifImage {
    ...
    /// Always [`ImageError::UnsupportedFormat`]: this build has no HEIF/HEIC encoder, so a
    /// buffer + metadata pair cannot be materialized into a HEIF/HEIC image. This is the funnel
...
impl ImageDecode for HeifImage {
    /// Always [`ImageError::UnsupportedFormat`]: this build has no HEIF/HEIC decoder.
