...
//!
//! Real per-platform runners (ONNX Runtime / CoreML / NNAPI) implement this trait behind the
//! **default-off, weight-fetching `inference` feature** (the real CLIP runner is a feature-gated
//! follow-up — it downloads weights at runtime, so it is never part of the default gate). Until
//! then [`FixtureRunner`] stands in — exactly as
...
//!
//! The runner works over **decoded pixels** conceptually (a real runner decodes + resizes the
//! frame bytes and runs the model); the fixture hashes the bytes. Two embedding tasks produce
//! stored vectors ([`SemanticSearch`](TaskKind::SemanticSearch),
//! [`FaceRecognition`](TaskKind::FaceRecognition)); the two detection tasks
//! ([`ObjectDetection`](TaskKind::ObjectDetection), [`FaceDetection`](TaskKind::FaceDetection))
//! produce boxes/landmarks that feed downstream stages.
//!
...
/// The inference seam every consumer routes through. Implemented by real per-platform runners
/// (behind the `inference` feature) and by [`FixtureRunner`] for tests.
...
mod tests {
    ...
    #[test]
    fn fixture_declares_canonical_models_for_both_embedding_and_detection_slots() {
        let r = FixtureRunner::new("cpu-reference");
        assert_eq!(
        ...
        assert_eq!(
            r.model(TaskKind::FaceRecognition),
            Some((ModelId::from("adaface"), ModelVersion::from("1")))
        );
        assert_eq!(
    ...
    #[test]
    fn face_recognition_is_an_embedding_task_the_fixture_serves() {
        // Face features ride the same embedding seam as semantic search (the S-H3 face slot).
        let r = FixtureRunner::new("cpu-reference");
        let v = r
            .embed_image(TaskKind::FaceRecognition, &[Frame::new(b"a face crop")])
            .unwrap();
