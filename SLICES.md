...
| Xcode 26.6 on the dev host | **broken — blocks the whole iOS lane locally** | `xcodebuild` cannot load `IDESimulatorFoundation`: a **March 2025** `/Library/Developer/PrivateFrameworks/DVTDownloads.framework` (v24224) shadows what Xcode 26.6 (installed July 2026) expects, so the symbol lookup fails. `-showsdks` still works, but anything loading the simulator plugin — including `xcodebuild -create-xcframework` — aborts. Everything upstream of it is fine: `build-ffi-apple` cross-compiles all three Apple targets, uniffi emits both `capsule_core_ffi.swift` and `capsule_sdk.swift`, and `lipo` produces the universal simulator slice; only the final packaging step fails. **Fix (needs sudo, outside the repo):** `sudo rm -rf /Library/Developer/PrivateFrameworks`, then re-run `mise run setup-swift`. Until then S-P2–S-P8 cannot be verified on this machine and must ride the `build-ios` CI lane. |
